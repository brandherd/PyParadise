...
class fit_linearComb(object):
    ...
    def __init__(self, basis, coeff=None):
        ...
        self._basis = basis
        if coeff is None:
            self._coeff = np.zeros(self._basis.shape[1], dtype=np.float32)
        else:
            self._coeff = coeff
    ...
    def __call__(self):
        ...
        `fit_linearComb.fit`.
        """
        return np.dot(self._basis, self._coeff)
    ...
    def chisq(self, y, sigma=1.0, mask=None):
        ...
        """
        if mask is None:
            valid_pix = np.ones(len(y), dtype="bool")
        else:
            valid_pix = np.logical_not(mask)
        return np.sum((((y-self())/sigma)**2)[valid_pix])
    ...
    def fit(self, y, sigma=1.0, mask=None, negative=False):
        ...
        """
        if len(sigma) == 0:
            error = np.ones(len(y), dtype=np.float32)
        else:
            error = sigma
        if mask is None:
            valid_pix = np.ones(len(y), dtype="bool")
            ...
            valid_pix = np.logical_not(mask)
        if not negative:
            (coeff_vector, residual) = optimize.nnls((self._basis/error[:, np.newaxis])[valid_pix, :], 
                                                     y[valid_pix]/error[valid_pix])
        else:
            (coeff_vector, residual, rank, s) = linalg.lstsq((self._basis/error[:, np.newaxis])[valid_pix, :], 
                                                             y[valid_pix]/error[valid_pix])
...
class fit_profile1D(object):
    ...
    def residuum(self, par, x, y, sigma=1.0):
        ...
        """
        self._par = par
        return np.sum(((y-self(x))/sigma)**2)
    ...
    def fit(self, x, y, sigma=1.0, p0=None, ftol=1e-8, xtol=1e-8, maxfev=9999, err_sim=0, 
            method='leastsq', parallel='auto'):
        ...
        if method == 'leastsq':
            try:
                model = optimize.leastsq(self.res, p0, (x, y, sigma), maxfev=maxfev, ftol=ftol, 
                                         xtol=xtol, full_output=0)
            except TypeError:
                model = optimize.leastsq(self.res, p0, (x, y, sigma), maxfev=maxfev, ftol=ftol, 
                                         xtol=xtol, full_output=0)
            self._par = model[0]
        ...
        if err_sim != 0:
            ...
            if parallel == 'auto':
                ...
                cpus = int(parallel)
            self._par_err_models = np.zeros((err_sim, len(self._par)), dtype=np.float32)
            if cpus > 1:
                ...
                for i in range(err_sim):
                    perr = deepcopy(self)
                    if method == 'leastsq':
                        results.append(pool.apply_async(optimize.leastsq, 
                                                        args=(perr.res, perr._par,
                                                              (x, np.random.normal(y, sigma), sigma),
                                                              None, 0, 0, ftol, xtol, 0.0, maxfev, 0.0, 100, None)))
                    if method == 'simplex':
                        results.append(pool.apply_async(optimize.fmin, 
                                                        args=(perr.residuum, perr._par,
                                                              (x, np.random.normal(y, sigma), sigma),
                                                              xtol, ftol, maxfev, None, 0, 0, 0)))
                    sleep(0.01)
                pool.close()
                pool.join()
                for i in range(err_sim):
                ...
                for i in range(err_sim):
                    ...
                    if method == 'leastsq':
                        try:
                            model_err = optimize.leastsq(perr.res, perr._par, (x, np.random.normal(y, sigma), sigma),
                    ...
                    if method == 'simplex':
                        try:
                            model_err = optimize.fmin(perr.residuum, perr._par, (x, np.random.normal(y, sigma), sigma),
                                                      disp=0, ftol=ftol, xtol=xtol)
                        except TypeError:
                            model_err = optimize.fmin(perr.residuum, perr._par, (x, np.random.normal(y, sigma), sigma),
                                                      disp=0, ftol=ftol, xtol=xtol)
                        self._par_err_models[i, :] = model_err

            self._par_err = np.std(self._par_err_models, 0)
        ...
        for n in self._names:
            if self._profile_type[n] == 'Gauss':
                ...
                if self._fixed[n]['vel'] == 1:
                    ...
                    select = np.logical_and(x > restwave*(vel/300000.0 + 1) - w/2.0,
                                            x < restwave*(vel/300000.0 + 1) + w/2.0)
                    idx = np.argsort(temp_y[select])
                ...
                if self._fixed[n]['disp'] == 1:
                    ...
                    select = np.logical_and(x > restwave*(vel/300000.0 + 1) - w/2.0,
                                            x < restwave*(vel/300000.0 + 1) + w/2.0)
                    try:
                        width = np.sqrt(np.sum((temp_y[select]*(x[select]-restwave*(vel/300000.0 + 1))**2)) /
                                        (np.sum(temp_y[select])))
                        if width > self._spec_res.getRes(wave_z) and np.isnan(width) is False:
                            disp = np.sqrt(width**2-self._spec_res.getRes(wave_z)**2)/(restwave*(vel/300000.0+1))\
                                   * 300000.0
    ...
    def _profile(self, x):
        ...
        for n in self._names:
            if self._profile_type[n] == 'Gauss':
                ...
                width = np.sqrt((disp/300000.0*wave)**2+self._spec_res.getRes(wave)**2)
                y += flux*np.exp(-0.5*((x-wave)/width)**2)/(fact*np.fabs(width))
        return y
...
class Gaussian(fit_profile1D):
    ...
    """
    def _profile(self, x):
        return self._par[0]*np.exp(-0.5*((x-self._par[1])/abs(self._par[2]))**2)/(fact*abs(self._par[2]))

    def _guess_par(self, x, y):
        sel = np.isfinite(y)
        dx = np.median(x[1:]-x[:-1])
        self._par[0] = np.sum(y[sel])
        self._par[1] = np.sum(x[sel]*y[sel])/self._par[0]
        self._par[2] = np.sqrt(np.sum((y[sel]*(x[sel]-self._par[1])**2))/self._par[0])
        self._par[0] *= dx
...
class Gaussian_const(fit_profile1D):
    ...
    """
    def _profile(self, x):
        return self._par[0]*np.exp(-0.5*((x-self._par[1])/self._par[2])**2)/(fact*self._par[2]) + self._par[3]

    def _guess_par(self, x, y):
        sel = np.isfinite(y)
        dx = np.median(x[1:]-x[:-1])
        ymin = np.min(y[sel])
        self._par[0] = np.sum(y[sel]-ymin)
        self._par[1] = np.sum(x[sel]*(y[sel]-ymin))/self._par[0]
        self._par[2] = np.sqrt(np.sum(((y[sel]-ymin)*(x[sel]-self._par[1])**2))/(self._par[0]))
        self._par[3] = ymin
        self._par[0] *= dx
...
class Gaussian_poly(fit_profile1D):
    ...
    """
    def _profile(self, x):
        return self._par[0]*np.exp(-0.5*((x-self._par[1])/self._par[2])**2)/(fact*self._par[2]) + \
               np.polyval(self._par[3:], x)
...
class Gaussians(fit_profile1D):
    ...
    def _profile(self, x):
        ...
        for i in range(ncomp):
            y += self._par[i]*np.exp(-0.5*((x-self._par[i+ncomp])/abs(self._par[i+2*ncomp]))**2) / \
...
class Gaussians_width(fit_profile1D):
    ...
    def _profile(self, x):
        ...
        for i in range(ncomp):
            y += self._par[i+1]*np.exp(-0.5*((x-self._args[i])/self._par[0])**2)/(fact*self._par[0])
        return y
...
class Gaussians_offset(fit_profile1D):
    ...
    def _profile(self, x):
        ...
        ncomp = len(self._args)
        for i in range(ncomp):
            y += self._par[i+1]*np.exp(-0.5*((x-self._args[i]+self._par[-1])/self._par[0])**2)/(fact*self._par[0])
        return y
...
class Gauss_Hermite(fit_profile1D):
    ...
    def _profile(self, x):
        ...
        y = a*np.exp(-0.5*w**2)*(1. + h3*H3 + h4*H4)/(fact*sigma)
        return y
...
class LegandrePoly(object):
    ...
    def __call__(self, x):
        ...
        x_poly = (x-self._min_x)*1.98/np.abs((np.abs(self._max_x)-np.abs(self._min_x)))-0.99
        for i in range(len(self._coeff)):
            y += self._poly[i](x_poly)*self._coeff[i]
        return y

    def fit(self, x, y):
        eigen_poly = np.zeros((len(x), len(self._coeff)), dtype=np.float32)
        for i in range(len(self._coeff)):
            self._coeff = np.zeros(len(self._coeff))
            self._coeff[i] = 1
            eigen_poly[:, i] = self(x)
#        print(eigen_poly, y)
        self._coeff = np.linalg.lstsq(eigen_poly, y)[0]