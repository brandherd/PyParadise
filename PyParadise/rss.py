...
class RSS(Data):
    ...
    def fit_Kin_Lib_simple(self, SSPLib, nlib_guess, vel_min, vel_max, disp_min, disp_max, min_y, max_y, mask_fit, 
                           iterations=2, mcmc_code='emcee', walkers=50, burn=1500, samples=4000, thin=2, verbose=False,
                           sample_out=False, parallel='auto'):
        ...
        def extract_result(results, j, sample_out):
            vel_fit[j] = results[0]
            vel_fit_err[j] = results[1]
            Rvel[j] = results[2]
            disp_fit[j] = results[3]
            disp_fit_err[j] = results[4]
            Rdisp[j] = results[5]
            fiber[j] = j
            fitted[j] = True
            coeff[j, :] = results[7]
            chi2[j] = results[8]
            if sample_out:
                vel_trace[j, :] = results[9]
                disp_trace[j, :] = results[10]
            rss_model[j, :] = results[6].unnormalizedSpec().getData()
            mask[j, :] = results[6].getMask()
            if verbose:
                print("Fitting of SSP(s) to fiber %d finished." % j)
            ...
            if spec.hasData() and m >= (min_y - 1) and m <= (max_y - 1):
                ...
                if cpus > 1:
                    results.append([m, pool.apply_async(spec.fitSuperposition, args,
                                                        callback=partial(extract_result, i=m))])
                    sleep(0.01)
                else:
                    try:
    ...
    def fit_Lib_Boots(self, lib_SSP, fiber, vel, disp, vel_err=None, disp_err=None, par_eline=None,
...
def loadRSS(infile, extension_data=None, extension_mask=None, extension_error=None):
    ...
    rss = RSS()
    rss.loadFitsData(infile, extension_data=extension_data, extension_mask=extension_mask,
