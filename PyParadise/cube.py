...
class Cube(Data):
    ...
    def fit_Kin_Lib_simple(self, SSPLib, nlib_guess, vel_min, vel_max, disp_min, disp_max, min_x, max_x, min_y,
                           max_y, mask_fit, iterations=2, mcmc_code='emcee', walkers=50, burn=1500, samples=4000,
                           thin=2, verbose=False, parallel='auto'):
        ...
        return vel_fit, vel_fit_err, Rvel, disp_fit, disp_fit_err, Rdisp, fitted, coeff,\
               chi2, x_pix, y_pix, cube_model, mask
    ...
    def fit_Lib_fixed_kin(self, SSPLib, nlib_guess, vel, vel_disp, x_pos, y_pos, min_x, max_x, min_y, max_y, mask_fit,
        ...
        for m in range(len(x_pos)):
            ...
            spec = self.getSpec(x, y)
            if spec.hasData() and x >= (min_x - 1) and x <= (max_x - 1) and y >= (min_y - 1) and y <= (max_y - 1):
                args = (SSPLib, nlib_guess, vel[m], vel_disp[m],
    ...
    def fitELines(self, par, select_wave, min_x, max_x, min_y, max_y, method='leastsq', guess_window=0.0,
                  spectral_res=0.0, ftol=1e-4, xtol=1e-4, verbose=1, parallel='auto'):
    ...
    def fit_Lib_Boots(self, lib_SSP, x_cor, y_cor, vel, disp, vel_err=None, disp_err=None, par_eline=None,
                      select_wave_eline=None, mask_fit=None, method_eline='leastsq', guess_window=10.0,
                      spectral_res=0.0, ftol=1e-4, xtol=1e-4, bootstraps=100, modkeep=80, parallel=1, verbose=False):
